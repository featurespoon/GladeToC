...
FileChoose2Gui *gui = g_new0 (FileChoose2Gui, 1);
	...
	gtk_window_set_transient_for (GTK_WINDOW(gui->file_choose2), GTK_WINDOW(((TopWindow *) owner)->gui->top_window));
	...
	gui->choose2 = (GtkButton*) gtk_dialog_add_button (GTK_DIALOG(gui->file_choose2), "Select", 5);
	gtk_widget_show (GTK_WIDGET (gui->choose2));
	g_object_set_data (G_OBJECT (gui->choose2), "owner", owner);


	gui->cancel2 = (GtkButton*) gtk_dialog_add_button (GTK_DIALOG(gui->file_choose2), "Cancel", 3);
